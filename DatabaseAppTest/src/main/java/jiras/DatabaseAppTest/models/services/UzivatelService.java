...
public class UzivatelService {
    ...
    public Uzivatel getUzivatelById(Long id) {
        return uzivatelRepository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Neplatné ID uživatele: " + id));
    }
    ...
    public Page<Uzivatel> getUzivatelePaged(Pageable pageable) {
        Page<Uzivatel> uzivatelePage = uzivatelRepository.findAllByAktivniTrue(pageable);
        // Zajištění, že nikdy nevrátíme null
        return uzivatelePage != null ? uzivatelePage : Page.empty(pageable);
    }
    ...
    public Page<Uzivatel> searchUzivatele(String keyword, Pageable pageable) {
        Page<Uzivatel> uzivatelePage = uzivatelRepository.findByAktivniTrueAndJmenoContainingIgnoreCaseOrPrijmeniContainingIgnoreCaseOrEmailContainingIgnoreCase(
